...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type_str: &str) -> Result<Chunk, PngMeError> {
        let chunk_type = ChunkType::from_str(chunk_type_str)?;
        let index = match self.get_chunk_by_chunk_type(&chunk_type) {
            Some(data) => data,
            None => return Err(PngMeError::ChunkTypeNotPresent),
        };
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        let chunk = match ChunkType::from_str(chunk_type) {
            Ok(chunk) => chunk,
            Err(_e) => return None,
        };
        match self.get_chunk_by_chunk_type(&chunk) {
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        const VEC_MIN_SIZE: usize = 10;
        const LENGTH_OF_OTHER_FIELDS: usize = 12;

        let png_data = match value.strip_prefix(&Self::STANDARD_HEADER) {
            Some(data) => data,
            None => return Err(Self::Error::IncorrectFileHeader),
        };

        let mut chunks: Vec<Chunk> = Vec::with_capacity(VEC_MIN_SIZE);
        let mut index: usize = 0;

        while index < png_data.len() {
            let data_length = u32::from_be_bytes(png_data[index..index + 4]
                .try_into()
                .unwrap());
            let start_index = index;
            index += data_length as usize + LENGTH_OF_OTHER_FIELDS;
            chunks.push(Chunk::try_from(&png_data[start_index..index])?);
        }
